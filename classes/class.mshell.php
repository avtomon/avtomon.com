...
class MShell
{
     ...
     */
    private function __construct (& $mc, int & $ttl, int & $tag_ttl, int & $delay, string & $solt, _PDO & $dbconnect)
    {
     ...
     * @param int $expire - время актуальности кэшированного результата выполнения запроса
     *
     * @return mixed
     * @throws Exception
     */
    public function getValue (string $query, array $params = [], int $expire = 0)
    {
                    ...
                    if ($value)
                    {
                        if ($value !== $this->lock_value)
                        {
                            ...
                            elseif ($tags = $this->dbconnect->getTables($query))
                            {
                                foreach ($tags as & $tag)
                                {
                                    if ($this->mc->get($tag) >= $value['ttl'] - $expire)
                                    {
                                        goto get;
                                    }
                                }
     ...
     * @param $expire - время актуальности элемента кэша
     *
     * @return bool
     * @throws MShellException
     */
    private function setValue (string & $key, $value, int & $expire)
    {
        if ($key)
        {
            $value = array('data' => $value, 'ttl' => time() + $expire);
     ...
     *
     * @param array $urls - адреса удаляемых страниц
     *
     * @return bool
     */
