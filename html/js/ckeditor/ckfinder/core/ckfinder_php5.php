...
class CKFinder
{
	...
	public static function SetupFCKeditor( &$editorObj, $basePath = CKFINDER_DEFAULT_BASEPATH, $imageType = null, $flashType = null, $videoType = null )
	{
		if ( empty( $basePath ) )
			$basePath = CKFINDER_DEFAULT_BASEPATH ;

		$ckfinder = new CKFinder( $basePath ) ;
		$ckfinder->SetupFCKeditorObject( $editorObj, $imageType, $flashType, $videoType );
	}

	// Non-static method of attaching CKFinder to FCKeditor
	public function SetupFCKeditorObject( &$editorObj, $imageType = null, $flashType = null, $videoType = null )
	{
		$url = $this->BasePath ;
		...
		$editorObj->Config['VideoBrowserURL'] = $url . $qs . 'type=' . ( empty( $videoType ) ? 'Video' : $videoType ) ;
		...
		$editorObj->Config['VideoUploadURL'] = $dir . urlencode( 'core/connector/php/connector.php?command=QuickUpload&type=') . ( empty( $videoType ) ? 'Video' : $videoType ) ;
	}

	// Static "SetupCKEditor".
	public static function SetupCKEditor( &$editorObj, $basePath = CKFINDER_DEFAULT_BASEPATH, $imageType = null, $flashType = null, $videoType = null )
	{
		if ( empty( $basePath ) )
			$basePath = CKFINDER_DEFAULT_BASEPATH ;

		$ckfinder = new CKFinder( $basePath ) ;
		$ckfinder->SetupCKEditorObject( $editorObj, $imageType, $flashType, $videoType );
	}

	// Non-static method of attaching CKFinder to CKEditor
	public function SetupCKEditorObject( &$editorObj, $imageType = null, $flashType = null, $videoType = null )
	{
		$url = $this->BasePath ;
		...
		$editorObj->config['filebrowserVideoBrowseUrl'] = $url . $qs . 'type=' . ( empty( $videoType ) ? 'Video' : $videoType ) ;
		...
		$editorObj->config['filebrowserVideoUploadUrl'] = $dir . 'core/connector/php/connector.php?command=QuickUpload&type=' . ( empty( $videoType ) ? 'Video' : $videoType ) ;
	}
}
